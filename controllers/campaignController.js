    ...
    try {
        ...
        if(!student) {
            return res.status(404).json({
                message: "Student not found, please create a student account to create campaign",
            });
        }

        const campaignIsActive = await campaignModel.findOne({studentId, isActive: true});

        if(campaignIsActive) {
            return res.status(400).json({
                message: "You already have an active campaign, please end your current campaign before creating a new one",
            });
        }
